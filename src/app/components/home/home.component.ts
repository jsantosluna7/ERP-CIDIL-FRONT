...
// 🎯 Importación corregida y tipada
import { LikeService } from '../../likes.service';
import { AnuncioService } from '../../anuncios.service';
import { AuthService } from '../../services/auth.service';
import { ComentarioService } from '../../comentarios.service'; // Path plural usado en tu código
...
  // Propiedad para el nombre del publicador que viene del backend
...
  curriculoOpen: Record<number, boolean> = {}; // Controla la visibilidad del formulario de CV
...
      this.esExterno = true;
      this.nombreUsuarioLogueado = 'Invitado'; // Saludo para no autenticados
      console.log('Auth: No autenticado → EXTERNO');
      return;
    }
...
    // 🚀 MODIFICACIÓN APLICADA: Prioriza 'nombreUsuario', luego 'nombre', luego el email.
    this.nombreUsuarioLogueado =
      usuario?.nombreUsuario || // 🥇 Intenta usar la propiedad 'nombreUsuario'
      usuario?.nombre || // 🥈 Intenta usar la propiedad 'nombre'
      this.authService.getEmail() || // 🥉 Último recurso: El correo
      'Usuario';

    // Asumimos que getRole() puede devolver el NOMBRE (string) o el ID (string/number)
    const rolRaw = this.authService.getRole()?.toUpperCase() ?? 'EXTERNO';
    this.rolActual = rolRaw;

    console.log('Auth: Rol detectado →', this.rolActual);

    // Lógica de rol prioriza el nombre, pero acepta el ID para compatibilidad
...
  /**
   * Determina si el usuario actual (autenticado) puede ver el Dashboard/Zona de Usuario.
   * Roles permitidos: 1, 2, 3, 4, 5.
   */
  navigateToDashboard(): void {
    if (!this.puedeVerDashboard()) {
      console.warn('Acceso denegado al Dashboard/Calendario para el rol actual.');
      return;
    }

    let targetRoute = '/home/dashboard'; // Ruta por defecto para SuperUser/Admin (Roles 1, 2)

    // 📅 MODIFICACIÓN APLICADA: Redirige a 'calendario' si es Profesor o Estudiante.
    if (this.esProfesor || this.esEstudiante) {
      targetRoute = '/home/calendario';
    }
    
    // Si es Personal CIDIL (Rol 5), también puede ir al dashboard.

    console.log(`Redirigiendo a: ${targetRoute} (Rol: ${this.rolActual})`);
...
    // Obtiene el rol (puede ser string o number)
    const rol = this.authService.getRole();

    // Usar nombres de rol y fallback a IDs numéricos si es necesario
    const rolesPermitidosStr = ['SUPERUSUARIO', 'ADMINISTRADOR', 'PROFESOR', 'ESTUDIANTE', 'PERSONALCIDIL'];
    const rolUpper = rol?.toUpperCase() ?? '';

    if (rolesPermitidosStr.includes(rolUpper)) {
        return true;
    }

    // Fallback a la verificación numérica (si el token aún usa IDs)
    const rolesPermitidosNum = [1, 2, 3, 4, 5];
    const rolNumerico = Number(rol);
    return rolesPermitidosNum.includes(rolNumerico);
  }
...
    this.rolActual = 'EXTERNO';
    this.esSuperUsuario = this.esAdmin = this.esProfesor = this.esEstudiante = this.esPersonalCidil = false;
    this.esExterno = true;
    this.nombreUsuarioLogueado = 'Invitado'; // Restablecer saludo
    this.router.navigate(['/anuncio']); // Redirigir a la ruta pública
  }
...
        this.anuncioService.obtenerTodos()
      );

      const baseUrl = this.imagenBaseUrl.endsWith('/')
        ? this.imagenBaseUrl
        : `${this.imagenBaseUrl}/`;
...
        // Mapear el nombre del publicador del backend (asumimos 'nombreUsuario')
        nombreUsuarioPublicador: a.nombreUsuario ?? 'Autor Desconocido',

        urlImagenes: a.imagenUrl
          ? a.imagenUrl.split(';').map((u: string) => {
...
        await Promise.all([
          this.cargarComentarios(anuncio),
          this.cargarLikes(anuncio),
        ]);
...
      this.mensajeErrorPublicar =
        'Faltan campos obligatorios (Título/Descripción).';
      return;
    }
...
    this.nuevasImagenes.forEach((img) =>
...
      // MODO EDICIÓN
      this.anuncioService
        .editarAnuncio(this.anuncioAEditar.id, formData)
        .subscribe({
          next: async (res: any) => {
            this.mensajeExitoPublicar =
              res?.mensaje ?? 'Anuncio actualizado correctamente ✅';
            await this.cargarAnuncios();
...
            this.mensajeErrorPublicar =
              err?.error?.error ?? 'Error al actualizar el anuncio.';
            this.mensajeExitoPublicar = null;
          },
        });
    } else {
...
        next: async (res: any) => {
          this.mensajeExitoPublicar =
            res?.mensaje ?? 'Anuncio publicado correctamente ✅';
          await this.cargarAnuncios();
          setTimeout(() => {
...
          this.mensajeErrorPublicar =
            err?.error?.error ?? 'Error al publicar el anuncio.';
          this.mensajeExitoPublicar = null;
        },
      });
    }
...
        this.comentarioService.obtenerPorAnuncio(anuncio.id)
      );
...
  // Manejo de la publicación con captura de errores
  publicarComentario(anuncio: Anuncio): void {
    const texto = (this.nuevoComentario[anuncio.id] ?? '').trim();

    if (!this.puedeComentarYDarLike()) {
      console.warn(
        'Debes iniciar sesión con una cuenta de Profesor o Estudiante para comentar.'
      );
      return;
    }
...
        } catch (e) {
          console.warn(
            'Advertencia: no se pudieron recargar los comentarios inmediatamente.',
            e
          );
        }
      },
...
        if (err?.status === 403) {
          console.warn(
            'No tienes permisos para comentar. Debes ser Profesor o Estudiante.'
          );
          return;
        }
...
          msg.toLowerCase().includes('id no encontrado')
        ) {
          console.warn(
            'Debes iniciar sesión con una cuenta de Profesor o Estudiante para comentar.'
          );
...
        this.likeService.contarLikes(anuncio.id)
      );
...
    this.likeService
      .darLike(anuncio.id, this.authService.getEmail() ?? 'anonimo')
      .subscribe({
        next: (res: any) => {
          anuncio.likes = res?.totalLikes ?? anuncio.likes;
          anuncio.usuarioDioLike =
            res?.usuarioDioLike ?? !anuncio.usuarioDioLike;
        },
        error: (err) => console.error(err),
...
        this.http.get<Curriculum[]>(`${this.baseUrl}/Curriculum`, {
...
      if (!nombre || !correo) {
        this.mensajeErrorCurriculo[anuncio.id] =
...
    try {
      const opciones: any = this.esExterno
        ? {}
        : { headers: this.getHeaders() };
      const res: any = await lastValueFrom(
        this.http.post(`${this.baseUrl}/Curriculum`, formData, opciones)
      );

      this.mensajeExitoCurriculo[anuncio.id] =
        res?.mensaje ?? 'Currículum subido ✅';
...
      this.mensajeErrorCurriculo[anuncio.id] =
        err?.error?.error ?? 'Error al subir currículum.';
      this.mensajeExitoCurriculo[anuncio.id] = null;
    }
  }
...
    this.http
      .delete(`${this.baseUrl}/Curriculum/${c.id}`, {
        headers: this.getHeaders(),
      })
      .subscribe({
        next: async () => {
          this.todosCurriculos = this.todosCurriculos.filter(
            (x) => x.id !== c.id
          );

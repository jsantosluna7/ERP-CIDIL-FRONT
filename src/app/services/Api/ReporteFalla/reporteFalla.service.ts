...
export class ReporteFallaService {
  ...
  actualizarReporte(
    id: number,
    dto: { IdReporte: number; estado: number },
  ): Observable<ReporteFalla> {
    return this.http.put<ReporteFalla>(`${this.apiUrl}/${id}`, dto);
  }

  getMiReportes(id: number): Observable<ReporteFalla[]> {
    return this.http.get<ReporteFalla[]>(
      `${this.apiUrl}/mis-reportes?usuario=${id}`,
    );
  }
}
